...
public class ResultView extends JPanel {
  ...
  protected void paintComponent(Graphics g) {
    ...
    Player me = store.state.game.getCurrentPlayer();
    ...
    List<Integer> stateMe = me.getCards();
    List<Integer> stateComputer = computer.getCards();
    for (int i = 0; i < stateMe.size(); i++) {
      myCards.add(new CardView(i, 1, stateMe.get(i)));
    }
    for (int i = 0; i < stateComputer.size(); i++) {
      computerCards.add(new CardView(i, 0, stateComputer.get(i)));
    }
    ...
    String result = computer.getCardSum() + " vs " + me.getCardSum();
    result += "  ";
