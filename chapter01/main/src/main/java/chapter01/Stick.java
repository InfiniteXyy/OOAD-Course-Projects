...
public class Stick {
    ...
    public void printFrame() {
        System.out.println(Arrays.toString(ants.stream().map(ant -> ant.getPosition()).toArray()));
    }

    public void initDirection(char[] directions) throws Exception {
        if (directions.length != ants.size()) throw new Exception("Directions numbers Error");
        for (int i = 0; i < ants.size(); i++) {
            if (directions[i] == '0')
                ants.get(i).collide();
        }
